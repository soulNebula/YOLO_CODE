...
class ImageCanvas(QLabel):
    ...
    bboxDrawn = pyqtSignal(int, int, int, int)  # x1, y1, x2, y2

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setMinimumSize(640, 480)
        self.setAlignment(Qt.AlignCenter)
        self.setStyleSheet("background-color: #fafafa; border: 2px solid #e0e0e0; border-radius: 4px;")
        self.setMouseTracking(True)

        self.original_image = None
        self.display_image = None
        self._render_buffer = None  # 保持numpy数组引用防止QImage悬空指针
        self.zoom = 1.0
        self.offset_x = 0
        self.offset_y = 0

        # 标注相关
        self.annotations = []  # [{class_id, cx, cy, w, h, x1, y1, x2, y2}]
        self.class_names = []
        self.colors = {}
    ...
    def _render(self):
        ...
        for idx, ann in enumerate(self.annotations):
            ...
            thick = 3 if idx == self.selected_bbox_idx else 2
            ...
            cv2.putText(img, label, (x1 + 3, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)

            if idx == self.selected_bbox_idx:
                cv2.rectangle(img, (x1 - 2, y1 - 2), (x2 + 2, y1 + 2), (255, 255, 0), 1)
        ...
        if self.drawing and self.draw_start and self.draw_end:
            ...
            cv2.line(img, (x1, y1), (x1 + 20, y1), color, 3)
            cv2.line(img, (x1, y1), (x1, y1 + 20), color, 3)
        ...
        # 保持引用防止QImage悬空指针（annotation.py已转为RGB）
        img_rgb = np.ascontiguousarray(img)
        self._render_buffer = img_rgb
        h2, w2 = self._render_buffer.shape[:2]
        q_img = QImage(self._render_buffer.data, w2, h2, w2 * 3, QImage.Format_RGB888)
    ...
    def _to_image_coords(self, pos):
        ...
        try:
            ...
            if dw <= 0 or dh <= 0 or ow <= 0 or oh <= 0 or self.zoom <= 0:
                return None, None

            x = int((pos.x() - self.offset_x - (self.width() - dw) // 2) / self.zoom)
            y = int((pos.y() - self.offset_y - (self.height() - dh) // 2) / self.zoom)
            x = max(0, min(ow - 1, x))
            y = max(0, min(oh - 1, y))
            return x, y
        except Exception:
    ...
    def mousePressEvent(self, event):
        ...
        try:
            ...
            self.selected_bbox_idx = -1
            for idx, ann in reversed(list(enumerate(self.annotations))):
                h, w = self.original_image.shape[:2]
                if 'x1' in ann:
            ...
            if self.drawing:
                ...
                for ann in self.annotations:
                    ...
                    if x1 <= img_x <= x2 and y1 <= img_y <= y2:
                        hovering = True
                        break
                self.setCursor(Qt.PointingHandCursor if hovering else Qt.CrossCursor)
        except Exception:
            pass
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Delete and self.selected_bbox_idx >= 0:
            if 0 <= self.selected_bbox_idx < len(self.annotations):
                self.annotations.pop(self.selected_bbox_idx)
                self.selected_bbox_idx = -1
                self._render()
...
class AnnotationWidget(QWidget):
    ...
    def _on_file_selected(self, item, col):
        ...
        try:
            ...
            self.canvas.load_image(img, annos)
            self._update_nav()
    ...
    def _load_image_at(self, idx):
        try:
            ...
            if self.manager.current_image is not None:
                self.canvas.load_image(self.manager.current_image, self.manager.get_current_annotations())
                self._update_nav()
                self._refresh_anno_table()
        except Exception:
            pass
    ...
    def _add_class(self):
        ...
        hue = (cls_id * 0.618) % 1.0
        r, g, b = hsv_to_rgb(hue, 0.8, 0.95)
        self.manager.class_colors[cls_id] = (int(r), int(g), int(b))
    ...
    def _on_bbox_drawn(self, x1, y1, x2, y2):
        ...
        self.canvas.annotations = self.manager.get_current_annotations()
        self.canvas._render()
        self._refresh_anno_table()
    ...
    def _refresh_anno_table(self):
        ...
        for i, ann in enumerate(annos):
            cls_id = ann.get('class_id', 0)
            cls_name = self.manager.classes[cls_id] if cls_id < len(self.manager.classes) else str(cls_id)
            h, w = 0, 0
            if self.manager.current_image is not None:
    ...
    def _save_annotations(self):
        ...
        QMessageBox.information(self, "提示", f"标注已保存到:\n{self.manager.dataset_dir}/labels/")
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        save_act.triggered.connect(self._switch_to_annotation)

...
| 模块 | 描述 |
|------|------|
| 📊 **仪表盘** | 项目概览、工作目录管理、系统状态、快捷操作、内存清理 |
| 🧭 **引导模式** | 6 步引导式流程：准备数据→标注→训练→推理→评估→导出 |
| 📝 **手动标注** | YOLO 格式标注，四角拖拽缩放，长宽比锁定，拖拽模式，标签过滤，质量检查 |
| 📁 **数据集** | 创建/导入数据集（复制到工作目录），自动生成 data.yaml |
| 🚀 **模型训练** | 环境检测修复、YOLO 训练、实时曲线、指标监控、数据增强预览、完整超参数 |
| 🔍 **模型推理** | 图片/视频/摄像头检测，Benchmark 速度对比，裁剪保存，多格式导出 |
...
不想手动摸索？点击侧边栏 **🧭 引导模式** 进入 6 步引导式流程：

```
✅ 准备数据 → ✅ 标注数据 → 🚀 训练模型 → 🔍 推理测试 → 📈 评估模型 → 📤 导出部署
```

- 每步完成后再进入下一步，已完成的显示绿色对勾
- 点击任意步骤的按钮即跳转到对应功能页面
